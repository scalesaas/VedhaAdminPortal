...
import { Booksbyadmin, updateBlogById } from "@/lib/actions/blog";

export default async function Bookstable() {
    const { data : Books } = await Booksbyadmin();
						...
						{Books?.map((Books, index) => {
							...
							return (
								<div className="grid grid-cols-3" key={index}>
									...
									<Actions id={Books.id } slug={Books.slug} />
								</div>
							);	
						})}
					</div>
...
const Actions = ({ id, slug }: { id: string; slug: string } ) => {
	return (
		<div className="flex items-center px-4 gap-2 md:flex-wrap">
			...
			<DeleteAlert id={id} />
			<Link href={`/dashboard/Books/edit/${slug}`}>
				<Button className="flex gap-2 items-center" variant="outline">
					<Pencil1Icon />
					Edit
				</Button>
			...
			<Link href={`/dashboard/Books/build/${slug}`}>
				<Button className="flex gap-2 items-center" variant="outline">
					<BsBuilding />
					Build
				</Button>
