...
import { readlinksByAdmin, updateBlogById } from "@/lib/actions/blog";
export default async function BlogTable() {

    const { data: links } = await readlinksByAdmin();
	return (
		<>
						...
						{links?.map((link, index) => {
							...
							return (
								<div className="grid grid-cols-5" key={index}>
									...
									<Actions id={link.id } slug={link.slug} />
								</div>
							);	
						})}
					</div>
...
const Actions = ({ id, slug }: { id: string; slug: string } ) => {
	return (
		<div className="flex items-center gap-2 md:flex-wrap">
			...
			<Link href={`/dashboard/link/edit/${slug}`}>
				<Button className="flex gap-2 items-center" variant="outline">
					<Pencil1Icon />
					Edit
				</Button>
