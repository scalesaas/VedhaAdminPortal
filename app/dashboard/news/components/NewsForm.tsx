...
interface NewsFormProps {
  defaultNews: INewsDetial;
  onHandleSubmit: (data: NewsFormSchemaType) => void;
}

export default function NewsForm({ onHandleSubmit, defaultNews }: NewsFormProps) {
  ...
  const form = useForm<NewsFormSchemaType>({
    mode: "all",
    defaultValues: {
      title: defaultNews?.title || "",
      image: defaultNews?.image || "",
      news_link: defaultNews?.news_link || "",
      status: defaultNews?.status || "",
      author: defaultNews?.author || "",
      created_at: defaultNews?.created_at || "",
    },
  });
  ...
  const onSubmit = (data: NewsFormSchemaType) => {
    startTransition(() => {
      onHandleSubmit(data);
    });
  };
